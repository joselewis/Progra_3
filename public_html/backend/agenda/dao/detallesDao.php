...
class DetallesDao {
    ...
    public function add(Detalles $detalles) {
        global $labAdodb;
        try {
            $sql = sprintf("insert into Detalls (idDetalle_Factura, SubTotal, IVA, Descuento, Facturacion_idFacturacion, Viaje_idViaje) 
                                          values (%s,%s, %s,%s,%s,%s)",
    ...
    public function update(Detalles $detalles) {
        ...
        global $labAdodb;
        try {
            $sql = sprintf("update Detalles set SubTotal = %s, 
                                                IVA = %s, 
                                                Descuento = %s, 
                                                Facturacion_idFacturacion = %s, 
                                                Viaje_idViaje = %s, 
            ...
            $valores["Viaje_idViaje"]            = $personas->getViaje_idViaje();
            $valores["idDetalle_Factura"]       = $personas->getidDetalle_Factura();
            $labAdodb->Execute($sqlParam, $valores) or die($labAdodb->ErrorMsg());
        } catch (Exception $e) {
